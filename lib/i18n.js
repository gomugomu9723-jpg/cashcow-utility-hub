...
const copy = {
  ko: {
    ...
    toolDescription: '{title}를 브라우저에서 바로 사용할 수 있습니다.',
    tools: mergeTools('ko'),
  },
  en: {
    ...
    toolDescription: 'Use {title} directly in your browser.',
    tools: mergeTools('en'),
  },
  ja: {
    ...
    toolDescription: '{title}をブラウザで直接使用できます。',
    tools: mergeTools('ja'),
  },
  zh: {
    ...
    toolDescription: '可直接在浏览器中使用{title}。',
    tools: mergeTools('zh'),
  },
  es: {
    ...
    toolDescription: 'Usa {title} directamente en tu navegador.',
    tools: mergeTools('es'),
  },
  fr: {
    ...
    toolDescription: 'Utilisez {title} directement dans votre navigateur.',
    tools: mergeTools('fr'),
  },
}
...
export function getToolDisplayTitle(locale, tool) {
  return getToolTranslation(locale, tool.slug, 'title') || tool.title
}
...
export function getToolDisplayDescription(locale, tool) {
  ...
  const template = getTranslation(locale, ['toolDescription'], 'Use {title} directly in your browser.')
  return template.replace('{title}', getToolDisplayTitle(locale, tool))
}
